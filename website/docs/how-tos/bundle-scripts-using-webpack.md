---
sidebar_label: "Bundle Your Scripts Using webpack"
title: "How to Bundle Your Scripts Using webpack"
description: "Introduce how to bundle your JS scripts using webpack."
---
...
I use the [webpack](https://webpack.js.org) to bundle some JS scripts today, see [openapi-scripts/pull/16](https://github.com/Oreoxmt/openapi-scripts/pull/16). The following lists the details of the repository and reasons for bundling:
...
- To use the [JS scripts](https://github.com/Oreoxmt/openapi-scripts/tree/main/src), I build a Docker image [`ghcr.io/oreoxmt/openapi-scripts`](https://github.com/Oreoxmt/openapi-scripts/pkgs/container/openapi-scripts).
- The Docker image is under frequently updated, and it takes a long time to pull the latest image due to 6 JS scripts.

## Step 1. Install the `webpack-cli`

To install the `webpack-cli`, run the following command. For more information about the directory structure, refer to [Basic setup](https://webpack.js.org/guides/getting-started/#basic-setup).
...
## Step 2. Configure and build the `webpack.config.js`
...
- The first stage is the `build` stage, which tells Docker to:

    - build an image starting with the node:16 image and name it `build`
    - copy all files in the repository to the `/build` folder

      :::tip
      To ignore some files, you can create a soft link from the `.gitignore` to the  `.dockerignore` using the `ln` command:
...
- The second stage is the result stage, which builds the image for production.

## Step 4. Run the Dockerfile

To build the image for testing, run the following command to build a `test` image:
